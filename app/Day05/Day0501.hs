...
moveCrate :: Int -> Int -> Supplies -> Supplies
moveCrate fromStack toStack supplies = fromMaybe supplies $ do
        from <- Map.lookup fromStack supplies
        to <- Map.lookup toStack supplies
        let (crate, newFrom) = splitAt 1 from
        let newTo = crate ++ to
        let newSupplies = Map.insert toStack newTo $ Map.insert fromStack newFrom $ supplies
        return newSupplies

makeMove :: Supplies -> Move -> Supplies
makeMove supplies (Move {amountOfCrates, fromStack, toStack }) = 
    let createMoves = take amountOfCrates $ repeat (moveCrate fromStack toStack)
        move = foldl (.) id createMoves
    in move supplies
