...
getSizes :: FileSystem -> [(FileSystem, Size, Total)]
getSizes file@(File _ size) = [(file, size, 0)]
getSizes directory@(Directory name children) = 
    let childrenSizes = concat $ fmap getSizes $ Map.elems children
        total = sum $ fmap getSize  childrenSizes
        subDirectories = filter (isDirectory . getFs) childrenSizes
    in 
        (directory, total, if total <= 100000 then total else 0 ) : subDirectories
            

solve fs =
    sum $ fmap getTotal  $ getSizes fs
