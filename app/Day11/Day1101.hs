    ...
    case Map.lookup monkeyId state of
        ...
        Just monkey -> 
            let (newMonkey, throws) = playTurn monkey
                ...
                state2 = foldl (\ state throw -> catch' throw state ) state1 throws
            in state2
...
inspectItem monkey = 
    case Seq.lookup 0 $ monkeyItems monkey of
        Nothing -> Nothing
        Just item -> 
            let worryLevel1 = evalOperation (monkeyOperation monkey) (worryLevel item)
                worryLevel2 = worryLevel1 `div` 3
                monkeyToThrowTo = runTest (monkeyTest monkey) worryLevel2
                newMonkey = monkey 
                    { monkeyItems = Seq.drop 1 $ monkeyItems monkey
                    }
                throw = (monkeyToThrowTo, Item worryLevel2)
