...
parseInputFile :: FilePath -> IO (Either ParseError [Round])
parseInputFile = parseFile p'
  where p' = round `sepBy` endOfLine >>= \ps -> eof >> return ps
...
main :: IO ()
main = do
  input <- parseInputFile "app/Day02/Input.txt"
